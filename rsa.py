...
print 'Extend...bam domain with template SOA jar'
#addTemplate(MW_HOME+'/oracle_common/common/templates/wls/oracle.wls-webservice-template.jar')
addTemplate('<%= @middleware_home_dir %>/soa/common/templates/wls/oracle.bam.server_template.jar')
#---------------------------------- CHANGE DATASOURCE ---------------------------------------#
...
print 'Add server group BAM12-MGD-SVRS to cluster'
for i in range(len(MANAGED_SERVER_MACHINES)):
    server = DOMAIN+'_ms0'+str(i+1)
    cd('/')
...
if CLUSTER:
    bamServers = getClusterServers(CLUSTER, ADMIN_SERVER)
